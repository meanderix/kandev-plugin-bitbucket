...
func matchWebhookPayload(product domain.Product, kind, event string, body []byte, cfg automationWebhookConfig) ([]byte, bool, error) {
	var payload webhookObject
	if err := json.Unmarshal(body, &payload); err != nil || payload == nil {
		return nil, false, fmt.Errorf("invalid event object")
	}
	...
	data, err := json.Marshal(map[string]any{"repository": repo, "branches": branches, "event_kind": kind})
